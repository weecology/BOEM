...
# as CropModel), run USGS_classification first and set SPLIT_DIR to that run's
# split dir (checkpoint_dir/buffer_30/<comet_id>). Otherwise we recompute split
# from annotations.
...
# Optional: set to e.g. "$CHECKPOINT_DIR/buffer_30/<comet_id>" to use CropModel's split
SPLIT_DIR="${BOEM_HIER_SPLIT_DIR:-}"

cd /blue/ewhite/b.weinstein/src/BOEM
if [[ -n "$SPLIT_DIR" && -f "$SPLIT_DIR/usgs_train_split.csv" && -f "$SPLIT_DIR/usgs_val_split.csv" ]]; then
  echo "Using CropModel split from $SPLIT_DIR (same test data)"
